...
export const PerplexityTestWidget: React.FC = () => {
  ...
  const getStatusBadge = () => {
    if (apiKeyStatus === 'checking') {
      return <Badge variant="secondary"><Loader2 className="w-3 h-3 mr-1 animate-spin" />Vérification...</Badge>;
    }
    if (apiKeyStatus === 'valid') {
      return <Badge className="bg-green-100 text-green-700"><CheckCircle className="w-3 h-3 mr-1" />API Configurée</Badge>;
    }
    return <Badge variant="destructive"><AlertCircle className="w-3 h-3 mr-1" />API Non Configurée</Badge>;
  };
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    ...
                    <span className="ml-2 font-mono">{testResult.response.content.length} chars</span>
                  </div>
                  <div>
