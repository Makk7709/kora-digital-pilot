...
describe('useHybridAI', () => {
    ...
    it('should orchestrate Perplexity analysis and ChatGPT generation', async () => {
      ...
      const mockChatGPTResponse = {
        ...
        json: async () => ({
          choices: [{
            message: {
              content: '🚀 L\'IA révolutionne le marketing digital ! Les nouvelles tendances montrent...'
            }
          }],
          usage: {
            prompt_tokens: 150,
  ...
  describe('summarizeText', () => {
    it('should summarize text using ChatGPT', async () => {
      const mockResponse = {
        ok: true,
        json: async () => ({
          choices: [{
  ...
  describe('analyzeOnly', () => {
    it('should perform analysis with Perplexity only', async () => {
      const mockAnalysis = {
        ...
        sources: [
          { title: 'Marketing Study', url: 'https://study.com', snippet: 'Résultats' }
        ],
