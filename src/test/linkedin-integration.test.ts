...
describe('LinkedIn Integration - Corrections TDD', () => {
  ...
  describe('🔧 Correction 1: Erreur 401 Profil Utilisateur', () => {
    test('SHOULD handle OAuth scopes correctly for profile access', async () => {
    ...
    test('SHOULD retry profile request with correct headers', async () => {
    ...
    test('SHOULD implement fallback strategy for profile errors', async () => {
      ...
      const mockFetch = vi.fn()
        ...
        .mockResolvedValueOnce({ // Deuxième tentative avec refresh
          ok: true,
          json: () => Promise.resolve({
            id: 'fallback-id',
  ...
  describe('🔧 Correction 3: Affichage des derniers articles', () => {
    test('SHOULD fetch and display recent posts with details', async () => {
      ...
      const mockPosts = [
        {
          id: 'post-1',
          content: 'L\'IA transforme notre approche du marketing digital. Découvrez comment Kora AI optimise vos campagnes...',
    ...
    test('SHOULD format post content and metrics correctly', async () => {
      ...
      const mockPost = {
        id: 'post-1',
        content: 'Very long content that should be truncated when displayed in the list view because it exceeds the maximum length allowed for preview',
        publishedAt: '2025-01-05T10:00:00Z',
        metrics: { impressions: 7500, clicks: 125, likes: 333, comments: 42, shares: 67 }
      };
  ...
  describe('🔧 Correction 4: Gestion d\'erreurs améliorée', () => {
    ...
    test('SHOULD implement retry mechanism for transient errors', async () => {
