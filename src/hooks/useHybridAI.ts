...
export const useHybridAI = () => {
   ...
   */
  const generateWithAnalysis = useCallback(
    async (request: AnalysisWithContentRequest): Promise<HybridResponse> => {
      setState((prev) => ({
        ...prev,
        isLoading: true,
   ...
   */
  const rewriteContent = useCallback(
    async (
  ...
  const analyzeOnly = useCallback(
    async (prompt: string, level: 'quick' | 'detailed' | 'comprehensive' = 'detailed') => {
