...
export const usePerplexity = (): UsePerplexityReturn => {
  ...
  const makePerplexityCall = useCallback(
    async <T>(
      operation: () => Promise<T>,
  ...
  const getBusinessInsights = useCallback(
    async (request: InsightRequest): Promise<PerplexityResponse | null> => {
  ...
  const getMarketingTrends = useCallback(
    async (topic: string, timeframe: '24h' | '7d' | '30d' = '7d'): Promise<MarketInsight[]> => {
  ...
  const getAITechWatch = useCallback(
    async (domain: string): Promise<PerplexityResponse | null> => {
      return makePerplexityCall(
        async () => {
          const service = getPerplexityService();
  ...
  const getCompetitorAnalysis = useCallback(
    async (competitors: string[], market: string): Promise<PerplexityResponse | null> => {
      return makePerplexityCall(
        async () => {
          const service = getPerplexityService();
        ...
        () => {
          const response = generateSimulatedResponse(
            `Analyse concurrentielle ${market}`,
            'competitor-analysis',
          );
          setState((prev) => ({ ...prev, lastResponse: response }));
          return response;
  ...
  const generateContentWithResearch = useCallback(
    async (
      topic: string,
      contentType: 'article' | 'post' | 'thread',
    ): Promise<PerplexityResponse | null> => {
  ...
  const findReliableSources = useCallback(
    async (topic: string, sourceTypes: string[] = []): Promise<PerplexityResponse | null> => {
      return makePerplexityCall(
        async () => {
          const service = getPerplexityService();
  ...
  useEffect(() => {
    if (state.isInitialized) {
      ...
      console.log(
        '📊 Cache stats: Actualisation manuelle uniquement pour protéger les crédits API',
      );
    }
        ...
        if (response) {
          ...
          for (const line of lines) {
            newAlerts.push({
              ...
              alert: line.trim(),
              timestamp: new Date(),
              priority: line.toLowerCase().includes('révolution')
                ? ('high' as const)
                : ('medium' as const),
            });
          }
