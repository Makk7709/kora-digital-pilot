...
describe('🔍 RealBrandIntelligenceService - Audit Complet', () => {
  ...
  describe('⚠️ Gestion d\'Erreurs', () => {
    it('doit gérer les erreurs d\'API Perplexity', async () => {
      ...
      await expect(
        service.generateRealDeepResearchReport(testBrandName)
      ).rejects.toThrow('Échec analyse Apple Inc.');
    });
...
describe('🔄 Tests d\'Intégration Perplexity', () => {
  let service: RealBrandIntelligenceService;
...
describe('📊 Métriques et Monitoring', () => {
  it('doit tracker les métriques d\'exécution', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('📈 Score de confiance')
    );
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('🕒 Fraîcheur données')
    );
  });
}); 