...
export class PDFExporterSections {
  ...
  addMarketPositioning(pdf: jsPDF, report: any, addTextPremium: Function, addPremiumBox: Function, addPremiumGradient: Function, checkPageBreakPremium: Function, addPremiumPageHeader: Function) {
    ...
    pdf.setLineWidth(1);
    pdf.line(matrixStartX, matrixStartY + quadrantSize, matrixStartX + (quadrantSize * 2), matrixStartY + quadrantSize);
    pdf.line(matrixStartX + quadrantSize, matrixStartY, matrixStartX + quadrantSize, matrixStartY + (quadrantSize * 2));
    ...
    addTextPremium('Relative Market Share', matrixStartX + quadrantSize - 10, matrixStartY + (quadrantSize * 2) + 10);
  ...
  addSectorialAnalysis(pdf: jsPDF, report: any, addTextPremium: Function, addPremiumBox: Function, addPremiumGradient: Function, checkPageBreakPremium: Function, addPremiumPageHeader: Function) {
    const pageHeight = pdf.internal.pageSize.height;
    const pageWidth = pdf.internal.pageSize.width;
    const margin = this.dimensions.margin;
    const contentWidth = pageWidth - (margin * 2);
    ...
    const forces = [
      { name: 'POUVOIR FOURNISSEURS', x: centerX, y: centerY - forceDistance, score: porterForces.suppliers },
      { name: 'POUVOIR CLIENTS', x: centerX, y: centerY + forceDistance, score: porterForces.buyers },
      { name: 'MENACE ENTRANTS', x: centerX - forceDistance, y: centerY, score: porterForces.newEntrants },
      { name: 'MENACE SUBSTITUTS', x: centerX + forceDistance, y: centerY, score: porterForces.substitutes }
    ];

    forces.forEach(force => {
      const boxColor = force.score >= 7 ? this.brandColors.corporateRed :
                      force.score >= 4 ? this.brandColors.premiumOrange :
    ...
    pdf.setTextColor(this.brandColors.corporateBlue[0], this.brandColors.corporateBlue[1], this.brandColors.corporateBlue[2]);
    pdf.setFont('helvetica', 'bold');
    addTextPremium('HEAT MAP SECTORIELLE - DYNAMIQUES CONCURRENTIELLES', margin + 10, currentY + 15);
    ...
    heatMapData.competitors.forEach((competitor, rowIndex) => {
      addTextPremium(competitor.name.substring(0, 10), startX - 15, startY + (rowIndex * cellSize) + 12);
      ...
      competitor.scores.forEach((score, colIndex) => {
        ...
        pdf.rect(startX + (colIndex * cellSize), startY + (rowIndex * cellSize), cellSize, cellSize, 'F');
        
        pdf.setDrawColor(this.brandColors.corporateGray[0], this.brandColors.corporateGray[1], this.brandColors.corporateGray[2]);
        pdf.rect(startX + (colIndex * cellSize), startY + (rowIndex * cellSize), cellSize, cellSize, 'S');
        
        pdf.setFontSize(10);
        pdf.setTextColor(intensity > 0.5 ? 255 : 0, intensity > 0.5 ? 255 : 0, intensity > 0.5 ? 255 : 0);
        pdf.setFont('helvetica', 'bold');
        addTextPremium(score.toString(), startX + (colIndex * cellSize) + 8, startY + (rowIndex * cellSize) + 15);
      });
    });
  }

  // === PAGES 8-9: BRAND EQUITY ASSESSMENT ===
  addBrandEquityAssessment(pdf: jsPDF, report: any, addTextPremium: Function, addPremiumBox: Function, addPremiumGradient: Function, checkPageBreakPremium: Function, addPremiumPageHeader: Function) {
    const pageHeight = pdf.internal.pageSize.height;
    const pageWidth = pdf.internal.pageSize.width;
    const margin = this.dimensions.margin;
    const contentWidth = pageWidth - (margin * 2);
    ...
    const gaugeColor = brandEquity.score >= 80 ? this.brandColors.emerald :
                      brandEquity.score >= 60 ? this.brandColors.premiumGold :
    ...
    let prevScoreX1 = 0, prevScoreY1 = 0, prevScoreX2 = 0, prevScoreY2 = 0;
    for (let i = 0; i <= scoreSteps; i++) {
      ...
      prevScoreX1 = x1; prevScoreY1 = y1; prevScoreX2 = x2; prevScoreY2 = y2;
    }
    ...
    const benchmarks = [
      ...
      { label: 'Performance relative', value: `+${brandEquity.score - brandEquity.sectorAverage}pts`, x: margin + 80 }
    ];
    ...
    dimensions.forEach((dimension, index) => {
      ...
      const barColor = dimension.score >= 80 ? this.brandColors.emerald :
                      dimension.score >= 60 ? this.brandColors.premiumGold :
                      this.brandColors.corporateRed;
  ...
  private generateBCGMatrix(report: any) {
    return {
  ...
  private generatePorterForces(report: any) {
    return {
