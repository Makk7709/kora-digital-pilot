...
PÉRIODE: Focus sur les 12 derniers mois avec impact potentiel sur ${brandName}.`;
  ...
  private parseRealRecentActions(content: string, brandName: string): RecentAction[] {
    ...
    sections.forEach(section => {
      if (section.trim().length > 50) { // Ignorer les sections trop courtes
        ...
        const dateMatch = actionText.match(/(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}|janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre|\d{1,2}\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{4})/i);
        
        const action: RecentAction = {
          date: dateMatch ? this.parseDate(dateMatch[0]) : new Date(),
          title: this.extractSection(actionText, '', actionText.split('.')[0] || actionText.substring(0, 100)),
  ...
  private extractPriorities(content: string): any[] {
    ...
    lines.forEach(line => {
      if ((line.includes('priorité') || line.includes('objectif') || line.includes('plan')) && line.length > 20) {
        priorities.push({
  ...
  private extractFutureProjections(content: string): string[] {
    ...
    const patterns = [
      /projections?\s*:?\s*([^\.]+)/gi,
      /perspectives?\s+d'avenir\s*:?\s*([^\.]+)/gi
    ];
  ...
  private extractDisruptionPotential(content: string): 'low' | 'medium' | 'high' {
    if (content.toLowerCase().includes('forte disruption') || content.toLowerCase().includes('révolution')) {
      return 'high';
    } else if (content.toLowerCase().includes('évolution') || content.toLowerCase().includes('changement')) {
      return 'medium';
    }
    return 'low';
  }
  ...
  private extractRevenue(content: string): number | undefined {
    const revenueMatch = content.match(/chiffre\s+d'affaires.*?(\d+(?:,\d+)?)\s*(?:milliards?|millions?)/i);
    return revenueMatch ? parseFloat(revenueMatch[1].replace(',', '.')) : undefined;
  }

  private extractProfitability(content: string): string {
    if (content.toLowerCase().includes('bénéfice') || content.toLowerCase().includes('profitable')) {
      return 'Profitable';
    } else if (content.toLowerCase().includes('perte') || content.toLowerCase().includes('déficit')) {
      return 'En perte';
    }
    return 'Stable';
  }
  ...
  private extractValuation(content: string): number | undefined {
    const valuationMatch = content.match(/valorisation.*?(\d+(?:,\d+)?)\s*(?:milliards?|millions?)/i);
  ...
  private parseDate(dateStr: string): Date {
    ...
    for (const format of formats) {
      ...
      if (match) {
        ...
        } else if (format === formats[2]) { // DD mois YYYY
  ...
  private estimateImpact(text: string): number {
    const lowerText = text.toLowerCase();
    if (lowerText.includes('majeur') || lowerText.includes('important') || lowerText.includes('révolutionnaire')) return 80;
  ...
  private identifyStakeholders(text: string): string[] {
    ...
    if (lowerText.includes('actionnaire') || lowerText.includes('investisseur')) stakeholders.push('Actionnaires');
    if (lowerText.includes('partenaire')) stakeholders.push('Partenaires');
    if (lowerText.includes('régulateur') || lowerText.includes('autorité')) stakeholders.push('Régulateurs');
  ...
  private determineScope(text: string): 'local' | 'national' | 'regional' | 'global' {
    const lowerText = text.toLowerCase();
    if (lowerText.includes('mondial') || lowerText.includes('global') || lowerText.includes('international')) return 'global';
