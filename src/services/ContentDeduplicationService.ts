...
export class ContentDeduplicationService {
  ...
  async deduplicateReportContent(report: any): Promise<any> {
    ...
    try {
      ...
      if (report.objectiveAnalysis) {
        cleanedReport.objectiveAnalysis = this.deduplicateObjectiveAnalysis(report.objectiveAnalysis);
      }
      
      if (report.strategicAnalysis) {
        cleanedReport.strategicAnalysis = this.deduplicateStrategicAnalysis(report.strategicAnalysis);
      }
  ...
  analyzeContentDuplication(content: string): DeduplicationResult {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      for (let j = i + 1; j < paragraphs.length; j++) {
        ...
        if (similarity >= this.SIMILARITY_THRESHOLD && paragraphs[i].length >= this.MIN_LENGTH_FOR_COMPARISON) {
          duplications.push({
            originalIndex: i,
            duplicateIndex: j,
            similarity,
    ...
    const qualityScore = this.calculateQualityScore(originalLength, cleanedContent.length, duplications.length);
    ...
    analyses.forEach(key => {
      if (cleaned[key]) {
        ...
        for (const [existingKey, existingContent] of processedContents) {
          ...
          if (similarity > 0.7) {
            console.log(`⚠️ Contenu similaire détecté entre ${key} et ${existingKey} (${(similarity*100).toFixed(1)}%)`);
  ...
  private deduplicateObjectiveAnalysis(analysis: any): any {
    ...
    if (cleaned.marketPosition) {
      cleaned.marketPosition = this.analyzeContentDuplication(cleaned.marketPosition).cleanedContent;
    }
    ...
    if (cleaned.brandHistory && cleaned.marketPosition) {
      ...
      if (similarity > 0.6) {
        console.log(`⚠️ Similitude détectée entre brandHistory et marketPosition (${(similarity*100).toFixed(1)}%)`);
        cleaned.marketPosition = this.extractUniqueContent(cleaned.marketPosition, cleaned.brandHistory);
      }
    }
   ...
   */
  private removeDuplicatedParagraphs(paragraphs: string[], duplications: ContentDuplication[]): string[] {
    const indicesToRemove = new Set(
      duplications
        .filter(d => d.recommendation === 'remove')
        .map(d => d.duplicateIndex)
    );
  ...
  private mergeSmartContent(content1: string, content2: string): string {
    ...
    // Ajouter tous les paragraphes uniques de content1
    for (const p1 of paragraphs1) {
      let bestMatch = -1;
      let bestSimilarity = 0;
      
      // Chercher le meilleur match dans content2
      for (let i = 0; i < paragraphs2.length; i++) {
        if (used2.has(i)) continue;
        
        const similarity = this.calculateSimilarity(p1, paragraphs2[i]);
      ...
      if (bestMatch >= 0) {
        // Merger les paragraphes similaires
        const merged = this.mergeTwoParagraphs(p1, paragraphs2[bestMatch]);
        mergedParagraphs.push(merged);
        used2.add(bestMatch);
      } else {
    ...
    // Ajouter les paragraphes restants de content2
    for (let i = 0; i < paragraphs2.length; i++) {
      if (!used2.has(i)) {
        mergedParagraphs.push(paragraphs2[i]);
      }
    }
  ...
  private extractUniqueContent(mainText: string, referenceText: string): string {
    ...
    const uniqueParagraphs = mainParagraphs.filter(mainP => {
      return !referenceParagraphs.some(refP => 
        this.calculateSimilarity(mainP, refP) > 0.7
      );
    });
    
    return uniqueParagraphs.length > 0 
   ...
   */
  private calculateQualityScore(originalLength: number, cleanedLength: number, duplicationsCount: number): number {
   ...
   */
  getDeduplicationStats(content: string): { duplications: number; uniqueWords: number; repetitionRate: number } {
