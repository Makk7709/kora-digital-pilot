...
class PerplexityProtectionMiddleware {
  ...
  public canMakeCall(endpoint: string = 'perplexity'): { allowed: boolean; reason?: string; retryAfter?: number } {
    ...
    const callsLastMinute = this.callHistory.filter(call => 
      (now - call.timestamp) < oneMinute
    ).length;
    ...
    const callsLastHour = this.callHistory.filter(call => 
      (now - call.timestamp) < oneHour
    ).length;
  ...
  public recordCall(endpoint: string = 'perplexity', cost: number = this.DEFAULT_LIMITS.costPerCall) {
    ...
    if (typeof window !== 'undefined' && (window as any).perplexityProtection) {
      (window as any).perplexityProtection.recordAPICall(endpoint, 'success', cost);
    }
  }
  ...
  public async protectedFetch(url: string, options: RequestInit = {}): Promise<Response> {
    ...
    if (!allowed) {
      ...
      throw new Error(`API call blocked: ${reason}. Retry after ${retryAfter ? Math.round(retryAfter / 1000) : 60}s`);
    }
  ...
  public getStats() {
    ...
    const callsLastDay = this.callHistory.filter(call => 
      (now - call.timestamp) < oneDay
    ).length;
...
export const usePerplexityProtection = () => {
  return {
    ...
    recordCall: (endpoint?: string, cost?: number) => perplexityProtection.recordCall(endpoint, cost),
    protectedFetch: (url: string, options?: RequestInit) => perplexityProtection.protectedFetch(url, options),
...
if (typeof window !== 'undefined') {
  (window as any).perplexityProtectionMiddleware = perplexityProtection;
} 