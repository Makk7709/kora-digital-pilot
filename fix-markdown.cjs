...
class MarkdownFixer {
  ...
  generateReport(results) {
    ...
    if (totalFixes > 0) {
      ...
      log.info('🧹 Conseil: Utilisez --clean pour supprimer les fichiers .backup');
    } else {
  ...
  cleanBackupFiles() {
    log.header('NETTOYAGE FICHIERS BACKUP');
    
    const backupFiles = this.findBackupFiles();
    
    if (backupFiles.length === 0) {
      log.info('Aucun fichier .backup trouvé');
      return { success: true, deleted: 0 };
    }

    log.info(`${backupFiles.length} fichiers .backup trouvés`);
    ...
    backupFiles.forEach(file => {
      try {
        ...
        errors++;
        log.error(`Erreur suppression ${file}: ${error.message}`);
      }
    });
    ...
    return { success: errors === 0, deleted };
  }
...
Options:
  ...
  --clean, -c        Nettoyer tous les fichiers .backup
...
Exemples:
  ...
  node fix-markdown.js --clean             # Nettoyer les fichiers .backup
...
async function main() {
  ...
  const cleanMode = args.includes('--clean') || args.includes('-c');
  
  const fixer = new MarkdownFixer();

  // Mode nettoyage
  if (cleanMode) {
    fixer.cleanBackupFiles();
    return;
  }
