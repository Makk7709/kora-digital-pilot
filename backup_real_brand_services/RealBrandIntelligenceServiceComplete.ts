/**
 * 🚀 REAL BRAND INTELLIGENCE SERVICE - VERSION COMPLÈTE ET FONCTIONNELLE
...
Priorise par impact potentiel et horizon temporel.`;
  ...
  // === MÉTHODES D'EXTRACTION MÉTRIQUES ===
...
Structure avec métriques quantifiées et recommandations stratégiques.`;
    ...
    return {
      ...
      weaknessesScore: this.extractScore(response.content, 'faiblesses|weaknesses', 40),
      opportunitiesScore: this.extractScore(response.content, 'opportunités|opportunities', 70),
      threatsScore: this.extractScore(response.content, 'menaces|threats', 45),
...
Identifie les alertes critiques nécessitant attention.`;
  ...
  private calculateRealConfidenceScore(objectiveAnalysis: ObjectiveAnalysis, recentActions: RecentAction[]): number {
    let score = 60; // Score de base plus conservateur
    
    // Bonus pour données historiques complètes (max +8)
    if (objectiveAnalysis.foundingYear && objectiveAnalysis.foundingYear > 1800) {
      score += 8;
    }
    
    // Bonus pour données financières (max +12)
    if (objectiveAnalysis.marketCapitalization && objectiveAnalysis.marketCapitalization > 0) {
      score += 12;
    }
    
    // Bonus pour nombre d'actions récentes (max +8)
    ...
    // Bonus pour actions récentes avec dates précises (max +5)
    const actionsWithDates = recentActions.filter(a => a.date);
    if (actionsWithDates.length >= 2) {
      score += 5;
    }
    
    // Bonus pour innovation et réputation scores (max +7)
    ...
    // Limitation à 95 maximum pour rester réaliste
  ...
  private extractScore(content: string, keyword: string, fallback: number): number {
    // Patterns plus robustes pour l'extraction de scores
    const patterns = [
      new RegExp(`${keyword}.*?(\\d{1,3})(?:/100|%|\\s*sur\\s*100)`, 'i'),
      new RegExp(`${keyword}.*?score.*?(\\d{1,3})`, 'i'),
      new RegExp(`${keyword}.*?(\\d{1,3})(?:\\s*points?)`, 'i'),
      new RegExp(`score.*?${keyword}.*?(\\d{1,3})`, 'i'),
      new RegExp(`(\\d{1,3}).*?${keyword}`, 'i')
    ];
    ...
    // Si aucun pattern ne fonctionne, chercher des mots-clés qualitatifs
    const lowerContent = content.toLowerCase();
    const lowerKeyword = keyword.toLowerCase();
    
    if (lowerContent.includes(lowerKeyword)) {
      // Analyse qualitative basée sur le contexte
      if (lowerContent.includes('excellent') || lowerContent.includes('exceptionnel')) return Math.min(95, fallback + 20);
      if (lowerContent.includes('très bon') || lowerContent.includes('fort')) return Math.min(85, fallback + 15);
      if (lowerContent.includes('bon') || lowerContent.includes('positif')) return Math.min(75, fallback + 10);
      if (lowerContent.includes('moyen') || lowerContent.includes('correct')) return Math.min(65, fallback + 5);
      if (lowerContent.includes('faible') || lowerContent.includes('bas')) return Math.max(30, fallback - 10);
    }
